...
```{r}
write.csv(clinic, "/home1/ethenche/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/ethenche/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 2. What is the data type of $gender? Categorical
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)? 50 years old
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this. Last followup date = 3767 and patient 1 is 20211 days old when the study starts, so the patient is 3767-(-20211) = 23978/365=65 years old when they last followed up.
...
main = "Scatter plot for age at diagnosis vs last contact days to",
xlab = "age of diagnosis(years)",
ylab = "last contact days to(days)",
col = "blue",
pch = "19")
```
...
Around 3%, but not quite sure because the graph does not have very detailed lables.
...
The p-value is 0.016, since it is smaller than 0.05, it is statistically significant. This tells us that older patients tends to have less survival rate than younger patients.
