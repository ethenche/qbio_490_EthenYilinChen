...
rna_counts <- rna_se@assays@data$unstranded# get rna_counts dataframe and write it as dataframe and read it, also substitute it's column names with the Tumor Sample Barcode of adapted_clinical which will be introduced later and its row names as the gene_id column at the rna_genes
...
```
Below is all the codes for the clinic part of the 
```{r}
...
adapted_methylation_cleaned$smoking_category <- ifelse(adapted_methylation_cleaned$smoking_category == "Heavy exposure", "Heavy exposure", ifelse(adapted_clinical_cleaned$smoking_category == "Non-smoker", "Non-smoker", "Neither"))
...
adapted_clinical_Heavy_exposure <- adapted_clinical_cleaned$smoking_category == "Heavy exposure"
adapted_methylation_Heavy_exposure <- adapted_methylation_cleaned$smoking_category == "Heavy exposure"
...
rna_Heavy_exposure <- as.numeric(unlist(rna_tpm_cleaned[gene_counts_mask, adapted_clinical_Heavy_exposure]))
methylation_Heavy_exposure <- (adapted_betas_cleaned[gene_betas_mask, adapted_methylation_Heavy_exposure])
...
```{r}
boxplot(rna_non_smoker, rna_Heavy_exposure, xlab='Group', ylab='Counts', names=c('Non_smoker', 'Heavy_exposure'), ylim=c(0, 1))
...
lines(1.3:(dim(methylation_non_smoker)[1]+0.3), apply(methylation_Heavy_exposure, 1, mean), col='red', type='h', lwd=3)
